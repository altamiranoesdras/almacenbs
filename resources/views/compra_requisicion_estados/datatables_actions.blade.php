@can('Ver Compra Requisicion Estados')
    <a href="{{ route('compra.requisiciones.estados.show', $id) }}" data-bs-toggle="tooltip" title="Ver" class='btn btn-icon btn-flat-secondary rounded-circle'>
        <i class="fa fa-eye"></i>
    </a>
@endcan

@can('Editar Compra Requisicion Estados')
    <a href="{{ route('compra.requisiciones.estados.edit', $id) }}" data-bs-toggle="tooltip" title="Editar" class='btn btn-icon btn-flat-info rounded-circle'>
