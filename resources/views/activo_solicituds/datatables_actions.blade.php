...
<a href="{{ route('activoSolicitudes.show', $id) }}" data-bs-toggle="tooltip" title="Ver" class='btn btn-icon btn-outline-secondary rounded-circle'>
    <i class="fa fa-eye"></i>
</a>
@endcan

@can('Editar Activo Solicitudes')
<a href="{{ route('activoSolicitudes.edit', $id) }}" data-bs-toggle="tooltip" title="Editar" class='btn btn-icon btn-outline-info rounded-circle'>
...
@can('anular ingreso de solicitud activos')
    @if($activoSolicitud->estado_id != \App\Models\ActivoSolicitudEstado::ANULADA && $activoSolicitud->estado_id == \App\Models\ActivoSolicitudEstado::INGRESADA )
        <a href="#" onclick="deleteItemDt(this)" data-id="{{$id}}" data-bs-toggle="tooltip" title="Anular Ingreso" class='btn btn-icon btn-outline-danger rounded-circle'>
