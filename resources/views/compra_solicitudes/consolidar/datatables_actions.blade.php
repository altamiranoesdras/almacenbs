@can('Ver Solicitud de Compra')
    <a href="{{ route('compra.solicitudes.show', $id) }}" data-toggle="tooltip" title="Ver" class='btn btn-icon btn-flat-secondary rounded-circle'>
        <i class="fa fa-eye"></i>
    </a>
@endcan

@can('Editar Solicitud de Compra')
    <a href="{{ route('compra.solicitudes.edit', $id) }}" data-toggle="tooltip" title="Editar" class='btn btn-icon btn-flat-info rounded-circle'>
