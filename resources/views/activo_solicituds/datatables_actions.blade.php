...
<a href="{{ route('activoSolicitudes.show', $id) }}" data-toggle="tooltip" title="Ver" class='btn btn-default btn-xs'>
    <i class="fa fa-eye"></i>
</a>
@endcan

@can('Editar Activo Solicitudes')
<a href="{{ route('activoSolicitudes.edit', $id) }}" data-toggle="tooltip" title="Editar" class='btn btn-info btn-xs'>
...
@can('anular ingreso de solicitud activos')
    @if($activoSolicitud->estado_id != \App\Models\ActivoSolicitudEstado::ANULADA && $activoSolicitud->estado_id == \App\Models\ActivoSolicitudEstado::INGRESADA )
        <a href="#" onclick="deleteItemDt(this)" data-id="{{$id}}" data-toggle="tooltip" title="Anular Ingreso" class='btn btn-danger btn-xs'>
