@can('Ver Compra Solicitud Estados')
    <a href="{{ route('compra.solicitudes.estados.show', $id) }}" data-toggle="tooltip" title="Ver" class='btn btn-icon btn-outline-secondary rounded-circle'>
        <i class="fa fa-eye"></i>
    </a>
@endcan

@can('Editar Compra Solicitud Estados')
    <a href="{{ route('compra.solicitudes.estados.edit', $id) }}" data-toggle="tooltip" title="Editar" class='btn btn-icon btn-outline-info rounded-circle'>
