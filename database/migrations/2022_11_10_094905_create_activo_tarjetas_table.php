...
class CreateActivoTarjetasTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('activo_tarjetas', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('colaborador_id')->nullable()->index('fk_activo_tarjetas_rrhh_colaboradores1_idx');
            $table->string('codigo', 45)->nullable();
            $table->string('codigo_interno', 45)->nullable();
            $table->integer('correlativo')->nullable();
            $table->boolean('impreso')->nullable();
            $table->unsignedBigInteger('usuario_crea')->index('fk_activo_tarjetas_users1_idx');
