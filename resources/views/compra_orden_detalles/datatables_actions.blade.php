@can('Ver Compra Orden Detalles')
    <a href="{{ route('compraOrdenDetalles.show', $id) }}" data-toggle="tooltip" title="Ver" class='btn btn-icon btn-flat-secondary rounded-circle'>
        <i class="fa fa-eye"></i>
    </a>
@endcan

@can('Editar Compra Orden Detalles')
    <a href="{{ route('compraOrdenDetalles.edit', $id) }}" data-toggle="tooltip" title="Editar" class='btn btn-icon btn-flat-info rounded-circle'>
