        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            {!! Form::open(['route' => 'rpt.kardex','method' => 'get']) !!}
                            <div class="form-row">


                                <div class="form-group col-sm-6">
                                    {!! Form::label('item','Artículo:') !!}
                                    {!! Form::select('item', $items, $item, ['id'=>'items','class' => 'form-control','multiple'=>"multiple",'style'=>'width: 100%']) !!}
                                </div>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-hover table-striped table-xtra-condensed" id="tbl-dets">
                                            ...
                                            @foreach($detalles as $det)
                                                <tr class="text-sm">
                                                    <td>{{diaSemana($det->fecha)}} {{fechaHoraLtn($det->fecha)}}</td>
                                                    <td>{{$det->numero}}</td>
                                                    <td class="text-uppercase">{{$det->cliente_prov}}</td>
                                                    <td>{{$det->tipo==1 ? $det->ingresos : ''}}</td>
                                                    <td>{{$det->tipo==2 ? $det->salidas : ''}}</td>
                                                    <td class="{{$loop->last ? 'text-bold' :''}}">{{$saldo+=$det->ingresos-=$det->salidas}}</td>
                                                    <td>{{$det->usuario}}</td>
                                                </tr>
                                            @endforeach
                                            </tbody>
